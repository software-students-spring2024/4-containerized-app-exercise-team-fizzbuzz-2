...
def test_multiple_audio_lengths():
    ...
    # using hugging face internal testing dataset to generate audio samples (both long and short) of about 5 seconds and 30 seconds
    short_audio = load_dataset("hf-internal-testing/librispeech_asr_demo", "clean", split="validation[0:1]", trust_remote_code=True)
    long_audio = load_dataset("hf-internal-testing/librispeech_asr_demo", "clean", split="validation[5:6]", trust_remote_code=True)
    ...
    # check if the transcription of the short audio is shorter than the transcription of the long audio
